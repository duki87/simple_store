<?php
  ...
  foreach ($result as $row) {
    ...
    $sub_array[] = $status;
    $sub_array[] = '<button type="button" name="update" id="'.$row["brand_id"].'" class="btn btn-info btn-xs update mx-auto" style="display:block" data-toggle="modal" data-target="#brandEditModal">Update</button>';
    $sub_array[] = '<button type="button" name="delete" id="'.$row["brand_id"].'" data-status="'.$row["brand_status"].'" class="'.$statusClass.'" style="display:block">'.$statusText.'</button>';
    $data[] = $sub_array;
  }
