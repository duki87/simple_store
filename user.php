...
<?php include('templates/editUserModal.php'); ?>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.update', function(event) {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function(data) {
        $('#user_new_name').val(data.user_name);
        $('#user_new_email').val(data.user_email);
        remClass();
      }
    });
  });
  ...
  $(document).on('click', '.delete', function() {
    ...
    if(confirm("Are you sure you want to change this user status?")) {
      $.ajax({
        url:  "user_action.php",
        method: "POST",
        data: {user_id:user_id,status:status,delete_user:delete_user},
        success: function(data) {
